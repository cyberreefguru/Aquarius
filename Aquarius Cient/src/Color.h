...
struct Color {
    ...
    /// @returns the Color::raw value for the given index
    ...
    inline Color(const Color& rhs) __attribute__((always_inline)) = default;

    /// Allow assignment from one RGB struct to another
    inline Color& operator= (const Color& rhs) __attribute__((always_inline)) = default;
    ...
    /// This allows testing a Color for zero-ness
    inline explicit operator bool() const __attribute__((always_inline))
    {
        return r || g || b;
    }

    /// Converts a Color to a 32-bit color having an alpha of 255.
